...
def main():
    ...
    row=next(r for r in rows if r['domain']=='code')
    tokens,pos=row['prompt_token_ids'],row['decode_position']
    rates=list(map(int,a.rates.split(',')))
    if a.steps<2 or a.future<1 or pos+a.steps+a.future>=len(tokens):
    ...
    identity=dict(schema='tail_live_decode_v1',manifest_sha256=sha(a.manifest),request_id=row['request_id'],
    ...
    base.atomic_json(a.output/'reference_gate.json',dict(logits_bit_exact=True,development_capture_bit_exact=True,**gate))
    del second
    ...
    for rate in rates:
        for policy in ['pr13','two_choice_oracle']:
            inherited=copy.deepcopy(prefix)
            points=[]
            choices=[]
            for step in range(a.steps):
                position=pos+step
                mapping={}
                current=execute(model,tokens[position],position,inherited,mapping)
                for layer in range(40):
                    cell=a.output/f'r{rate}'/policy/f'step_{step}'/f'layer_{layer:02d}'
                    ...
                    if not (bp.exists() and mp.exists()):
                        options,facts=live.prepare(cap,layer,rate,a.checkpoint,a.trees,a.fit_dir,factors[str(layer)])
                        values=dict(cap)
                        records=[]
                        ...
                        save_npz(bp,values)
                        base.atomic_json(mp,dict(bank_sha256=sha(bp),terminal_labels_read=False,**facts))
                    ...
                    if cp.exists():
                        ...
                        if choice['bank_sha256']!=meta['bank_sha256']:
                            raise ValueError('live choice refers to a different bank')
                    else:
                        evaluated=[]
                        for record in records:
                            ...
                            candidate=execute(model,tokens[position],position,inherited,trial)
                        ...
                        choice=dict(bank_sha256=meta['bank_sha256'],chosen=winner['kind'],outcomes=evaluated,
                                    rate=rate,policy=policy,step=step,layer=layer)
                        base.atomic_json(cp,choice)
                    kind=choice['chosen']
                    state=bank[kind+'_state']
                    if live.frozen.state_page_count(state)>8*rate:
                        raise RuntimeError('live selected state exceeds payload')
                    mapping[layer]=(cap[f'routed_{layer}'],bank[kind+'_delta'].copy())
                    del current
                    current=execute(model,tokens[position],position,inherited,mapping)
                    ...
                    choices.append(dict(step=step,layer=layer,kind=kind,pages=live.frozen.state_page_count(state)))
                    print('live',rate,policy,step,layer,kind,quality['kl'],flush=True)
                inherited=current.past_key_values
                points.append(dict(step=step,mode='all_layers_compressed',**base.quality(refs[step],current.logits,tokens[position+1])))
                del current
                base.atomic_json(a.output/f'r{rate}'/policy/f'step_{step}_complete.json',points[-1])
            for step in range(a.steps,a.steps+a.future):
                current=base.forward(model,[tokens[pos+step]],inherited,pos+step)
                inherited=current.past_key_values
                points.append(dict(step=step,mode='exact_decode_after_compressed_history',
                                   **base.quality(refs[step],current.logits,tokens[pos+step+1])))
                del current
            trajectory=dict(rate=rate,policy=policy,request_id=row['request_id'],domain=row['domain'],points=points,choices=choices)
            base.atomic_json(a.output/f'r{rate}'/policy/'trajectory.json',trajectory)
    ...
    base.atomic_json(a.output/'completed.json',dict(completed=True,trajectories=len(trajectories),
        ...
        peak_gpu_gib=torch.cuda.max_memory_allocated()/2**30,cpu_expert_cache_bytes=live._CACHE_BYTES))
