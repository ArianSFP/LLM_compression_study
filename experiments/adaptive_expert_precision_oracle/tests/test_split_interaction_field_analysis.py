...
def _bundle(tmp_path: Path):
    ...
    for invocation in range(69):
        identity = {
            ...
            "request_id": f"req-{invocation // 23}",
            "position": invocation,
            "layer": [0, 4, 20, 39][invocation % 4],
            "expert_id": invocation % 3,
        }
        for budget in config["page_budgets"]:
            state_text, count_text = _states(budget)
            for factor_index, factor in enumerate(factors):
                factor_bytes = 2052 + factor_index * 100
                reference_recovery = .94 + invocation * 1e-5
                common = {
                    **identity,
                    "physical_budget_pages": budget,
                    "physical_pages": budget,
                    "physical_budget_bpw": budget / 768,
                    "factor_config_id": factor,
                    "factor_family": "joint_eigh",
                    "total_rank": 8,
                    "factor_payload_bytes": factor_bytes,
                    "combined_metadata_bytes": factor_bytes + 3084,
                    "combined_metadata_bpw": 8 * (factor_bytes + 3084) / 3145728,
                    "pr11_four_state_recovery": reference_recovery,
                    "pr10_exact_hybrid_recovery": .95,
                    "selected_states": state_text,
                    "state_counts": count_text,
                    "split_only_units": 0,
                    "gate_high_units": budget // 3,
                    "up_high_units": budget // 3,
                    "down_high_units": budget // 3,
                    "coordinate_sweeps": 8,
                    "local_evaluated_passes": 4,
                    "selector_runtime_ms": 2.0,
                    "exact_gram_oracle": False,
                    "selection_regime": "h0_exact_mixed_h4_split_action_geometry_ceiling",
                }
                for solver, gain, macs in (
                    (config["four_state_reference_solver_id"], 0.0, 800000),
                    (config["primary_solver_id"], .002, 1200000),
                    (config["warm_start_solver_id"], .003, 1800000),
                ):
                    recovery = reference_recovery + gain
                    rows.append({
                        **common,
                        "solver": solver,
                        "selected_seed": "synthetic",
                        "recovery": recovery,
                        "four_state_reference_recovery": reference_recovery,
                        "recovery_gain_vs_four_state_reference": gain,
                        "four_state_reference_kind": "same_factor_pr11",
                        "set_gain_retention_vs_pr10_exact_hybrid": recovery / .95,
                        "compressed_predicted_damage": 1 - recovery,
                        "exact_qenergy_damage": 1 - recovery,
                        "damage_prediction_error_over_base": 0.0,
                        "selector_compute_macs": macs,
                        "selector_compute_gate_pass": macs < 1572864,
                        "selector_compute_accounting_bound": "complete",
                        "promotable": (
                            solver == config["primary_solver_id"]
                            and factor in primary_factors
                        ),
                        "field_build_macs": 1,
                        "coordinate_macs": 1,
                        "local_search_macs": 1,
            ...
            for solver, gain in (
                (config["exact_four_state_solver_id"], 0.0),
                (config["exact_eight_state_solver_id"], .004),
            ):
                ...
                rows.append({
                    **identity,
                    ...
                    "factor_family": "exact_full_down_gram",
                    "total_rank": np.nan,
                    "factor_payload_bytes": np.nan,
                    "combined_metadata_bytes": np.nan,
                    "combined_metadata_bpw": np.nan,
                    "solver": solver,
                    "selected_seed": "synthetic",
                    "recovery": recovery,
                    "four_state_reference_recovery": exact_four,
                    "pr11_four_state_recovery": .90,
                    "recovery_gain_vs_four_state_reference": gain,
                    ...
                    "damage_prediction_error_over_base": 0.0,
                    "selector_runtime_ms": 2.0,
                    "selector_compute_macs": np.nan,
                    "selector_compute_gate_pass": np.nan,
                    "selector_compute_accounting_bound": "oracle",
                    "coordinate_sweeps": 8,
                    "local_evaluated_passes": 4,
                    ...
                    "promotable": False,
                    "selection_regime": "h0_exact_mixed_h4_split_action_geometry_ceiling",
                    "selector_non_mac_operations": "control",
                    "selected_states": state_text,
                    "state_counts": count_text,
                    "split_only_units": 0,
                    "gate_high_units": budget // 3,
