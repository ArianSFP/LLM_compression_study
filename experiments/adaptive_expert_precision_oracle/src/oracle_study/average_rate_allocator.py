...
@dataclass(frozen=True)
class AllocationAwareFrontierTrace:
    """Frontiers and work after repairing allocator-selected columns."""
...
class AllocationTrace:
    ...
    exchange_evaluations: int = 0


@dataclass(frozen=True)
class GlobalBoundTrace:
    """A valid global lower bound on exact combined-group damage."""
...
def _selected_state_signature(
    frontiers: Sequence[Sequence[RateOption]], allocation: AllocationTrace,
) -> tuple[tuple[int, tuple[int, ...]], ...]:
    return tuple(
        (
            int(frontiers[expert][int(option)].pages),
            tuple(frontiers[expert][int(option)].states.tolist()),
        )
        for expert, option in enumerate(allocation.option_indices)
    )


def _marginal_price(
    frontier: Sequence[RateOption], selected_index: int,
) -> float | None:
    current = frontier[int(selected_index)]
    slopes: list[float] = []
    if selected_index > 0:
        lower = frontier[selected_index - 1]
        width = int(current.pages) - int(lower.pages)
        if width > 0:
            slopes.append((float(lower.damage) - float(current.damage)) / width)
    if selected_index + 1 < len(frontier):
        higher = frontier[selected_index + 1]
        width = int(higher.pages) - int(current.pages)
        if width > 0:
            slopes.append((float(current.damage) - float(higher.damage)) / width)
    positive = [value for value in slopes if np.isfinite(value) and value > 0.0]
    return None if not positive else float(np.exp(np.mean(np.log(positive))))


def allocation_aware_rate_frontiers(
    ...
    *,
    ...
    local_max_passes: int,
    max_rounds: int = 3,
    adaptive_price_multipliers: Sequence[float] = (0.5, 2.0),
    adaptive_price_local_passes: int = 0,
) -> AllocationAwareFrontierTrace:
    """Repair allocator-selected columns and adaptively sample nearby prices.

    Each round solves the current MCKP, fully repairs only its selected hard
    page points, inserts two local price probes around the selected marginal
    slope, Pareto-prunes, and resolves.  Convergence is defined by the exact
    selected state vectors, not by unstable option indices.
    """

    if len(fields) != len(frontiers) or not fields:
        raise ValueError("fields and frontiers must be nonempty and aligned")
    burst = int(burst_cap_pages)
    if burst < 0:
        raise ValueError("burst cap must be nonnegative")
    multipliers = tuple(float(value) for value in adaptive_price_multipliers)
    if any(not np.isfinite(value) or value <= 0.0 for value in multipliers):
        raise ValueError("adaptive price multipliers must be positive finite")
    working = tuple(_pareto_options([
        option for option in frontier if int(option.pages) <= burst
    ]) for frontier in frontiers)
    allocation = multiple_choice_allocate(working, int(page_budget), weights)
    rounds = selected_repairs = price_solves = sweeps = local_passes = 0
    for round_index in range(int(max_rounds)):
        before = _selected_state_signature(working, allocation)
        updated: list[tuple[RateOption, ...]] = []
        for expert, field in enumerate(fields):
            selected = int(allocation.option_indices[expert])
            option = working[expert][selected]
            coordinate = split_coordinate_descent(
                field, option.states, int(option.pages),
                max_sweeps=int(coordinate_sweeps),
            )
            repaired = split_local_search(
                field, coordinate.states, int(option.pages),
                shortlist_size=int(local_shortlist),
                max_swap_units=int(local_swap_units),
                max_passes=int(local_max_passes),
            )
            additions = [RateOption(
                pages=int(repaired.pages),
                damage=float(repaired.damage),
                states=repaired.states.copy(),
                source=f"selected_hard_repair_round_{round_index + 1}",
                page_price=0.0,
            ...
            if center is not None:
                for multiplier in multipliers:
                    price = center * multiplier
                    priced = split_coordinate_descent(
                        field, repaired.states, burst, page_price=price,
                        max_sweeps=int(coordinate_sweeps),
                    )
                    priced_repair = split_local_search(
                        field, priced.states, burst, page_price=price,
                        shortlist_size=int(local_shortlist),
                        max_swap_units=int(local_swap_units),
                        max_passes=int(adaptive_price_local_passes),
                    )
                    additions.append(RateOption(
                        pages=int(priced_repair.pages),
                        damage=float(priced_repair.damage),
                        states=priced_repair.states.copy(),
                        source=f"adaptive_price_round_{round_index + 1}",
                        page_price=float(price),
                        coordinate_sweeps=int(priced.sweeps),
                        local_passes=int(priced_repair.passes),
                    ))
                    price_solves += 1
                    sweeps += int(priced.sweeps)
                    local_passes += int(priced_repair.passes)
            updated.append(_pareto_options((*working[expert], *additions)))
        working = tuple(updated)
        allocation = multiple_choice_allocate(working, int(page_budget), weights)
        rounds = round_index + 1
        if _selected_state_signature(working, allocation) == before:
            break
    return AllocationAwareFrontierTrace(
        frontiers=working,
        allocation=allocation,
        rounds=rounds,
        selected_repairs=selected_repairs,
        adaptive_price_solves=price_solves,
...
def _weighted_option_features(
    ...
    count = len(frontiers)
    alpha = np.asarray(router_weights, np.float64).reshape(-1)
    if alpha.shape != (count,) or np.any(~np.isfinite(alpha)):
        raise ValueError("router weights must be finite and match frontiers")
    result = []
    for expert, (frontier, raw) in enumerate(zip(frontiers, option_features)):
        values = np.asarray(raw, np.float64)
        if values.ndim != 2 or values.shape[0] != len(frontier):
            raise ValueError("option features do not match frontier")
        result.append(values * alpha[expert])
    return tuple(result)


def _linear_multiple_choice_allocate(
    frontiers: Sequence[Sequence[RateOption]],
    scores: Sequence[np.ndarray],
    page_budget: int,
) -> AllocationTrace:
    """Solve a finite-frontier linear objective under the pooled page cap."""
    count, budget = len(frontiers), int(page_budget)
    if len(scores) != count or budget < 0:
        raise ValueError("linear MCKP inputs are inconsistent")
    dynamic = np.full(budget + 1, np.inf, np.float64)
    dynamic[0] = 0.0
    choices = np.full((count, budget + 1), -1, np.int32)
    previous = np.full((count, budget + 1), -1, np.int32)
    for expert, frontier in enumerate(frontiers):
        values = np.asarray(scores[expert], np.float64)
        if values.shape != (len(frontier),) or np.any(~np.isfinite(values)):
            raise ValueError("linear option scores are invalid")
        updated = np.full_like(dynamic, np.inf)
        finite = np.flatnonzero(np.isfinite(dynamic))
        for option_index, option in enumerate(frontier):
            pages = int(option.pages)
            source = finite[finite + pages <= budget]
            target = source + pages
            candidate = dynamic[source] + float(values[option_index])
            better = candidate < updated[target] - 1e-15
            ties = np.abs(candidate - updated[target]) <= 1e-15
            better |= ties & (
                (choices[expert, target] < 0)
                | (option_index < choices[expert, target])
            )
            selected = np.flatnonzero(better)
            if selected.size:
                positions = target[selected]
    ...
    if not len(feasible):
        raise RuntimeError("linear MCKP has no feasible allocation")
    used = min(feasible.tolist(), key=lambda pages: (float(dynamic[pages]), pages))
    selected = np.empty(count, np.int64)
    cursor = int(used)
    for expert in range(count - 1, -1, -1):
        selected[expert] = int(choices[expert, cursor])
        cursor = int(previous[expert, cursor])
    return AllocationTrace(selected, int(used), float(dynamic[used]))


def bounded_group_exchange_allocate(
    frontiers: Sequence[Sequence[RateOption]],
    option_features: Sequence[np.ndarray],
    router_weights: Sequence[float] | np.ndarray,
    page_budget: int,
    ...
    *,
    exchange_sizes: Sequence[int] = (3, 4),
    shortlist_size: int = 8,
    max_passes: int = 2,
    tolerance: float = 1e-12,
) -> AllocationTrace:
    """Improve an exact-objective seed with bounded three/four-expert moves.

    Every Cartesian product over each deterministic shortlist is evaluated
    exactly through scalar Gram identities.  This is an upper-bound search;
    global certification is supplied separately by global_group_dual_bound.
    """
    weighted = _weighted_option_features(frontiers, option_features, router_weights)
    count, budget = len(frontiers), int(page_budget)
    selected = np.asarray(seed.option_indices, np.int64).copy()
    pages = int(sum(
        int(frontiers[e][int(selected[e])].pages) for e in range(count)
    ))
    if pages > budget:
        raise ValueError("exchange seed exceeds the page budget")
    residual = sum(
        (weighted[e][int(selected[e])] for e in range(count)),
        np.zeros(weighted[0].shape[1], np.float64),
    )
    objective = float(residual @ residual)
    evaluations = passes = 0
    sizes = tuple(sorted({int(value) for value in exchange_sizes}))
    if any(value < 2 or value > count for value in sizes):
        raise ValueError("exchange sizes lie outside the expert group")
    keep = max(1, int(shortlist_size))
    for pass_index in range(int(max_passes)):
        best = None
        for size in sizes:
            for subset in combinations(range(count), size):
                old = tuple(int(selected[e]) for e in subset)
                base = residual - sum(
                    (weighted[e][old[pos]] for pos, e in enumerate(subset)),
                    np.zeros_like(residual),
                )
                base_pages = pages - sum(
                    int(frontiers[e][old[pos]].pages)
                    for pos, e in enumerate(subset)
                )
                shortlists = []
                for pos, expert in enumerate(subset):
                    without = residual - weighted[expert][old[pos]]
                    candidates = without[None, :] + weighted[expert]
                    energy = np.einsum(
                        "ij,ij->i", candidates, candidates, optimize=True,
                    )
                    order = sorted(
                        range(len(frontiers[expert])),
                        key=lambda index: (
                            float(energy[index]),
                            int(frontiers[expert][index].pages),
                            index,
                        ),
                    )
                    chosen = order[:keep]
                    if old[pos] not in chosen:
                        chosen[-1] = old[pos]
                    shortlists.append(tuple(sorted(set(chosen))))
                base_energy = float(base @ base)
                singles = {
                    expert: (
                        2.0 * (weighted[expert] @ base)
                        + np.einsum(
                            "ij,ij->i", weighted[expert], weighted[expert],
                            optimize=True,
                        )
                    )
                ...
                cross = {
                    (left, right): 2.0 * (
                        weighted[left] @ weighted[right].T
                    )
                    for left, right in combinations(subset, 2)
                }
                for choices in product(*shortlists):
                    candidate_pages = base_pages + sum(
                        int(frontiers[e][choices[pos]].pages)
                        for pos, e in enumerate(subset)
                    )
                    if candidate_pages > budget:
                        continue
                    energy = base_energy + sum(
                        float(singles[e][choices[pos]])
                        for pos, e in enumerate(subset)
                    )
                    energy += sum(
                        float(cross[(left, right)][
                            choices[subset.index(left)],
                            choices[subset.index(right)],
                        ])
                        for left, right in combinations(subset, 2)
                    )
                    evaluations += 1
                    key = (
                        float(energy), int(candidate_pages), subset,
                        tuple(map(int, choices)),
                    )
                    if best is None or key < best[0]:
                        best = (key, subset, tuple(map(int, choices)))
        passes = pass_index + 1
        if best is None or best[0][0] >= objective - float(tolerance):
            break
        _, subset, choices = best
        for pos, expert in enumerate(subset):
            selected[expert] = choices[pos]
        pages = int(sum(
            int(frontiers[e][int(selected[e])].pages) for e in range(count)
        ))
        residual = sum(
        ...
        objective = float(residual @ residual)
    return AllocationTrace(
        selected.copy(), pages, objective,
        int(seed.coordinate_sweeps), int(seed.pair_passes),
        passes, evaluations,
    )


def global_group_dual_bound(
    frontiers: Sequence[Sequence[RateOption]],
    option_features: Sequence[np.ndarray],
    router_weights: Sequence[float] | np.ndarray,
    page_budget: int,
    upper_allocation: AllocationTrace,
    *,
    max_iterations: int = 128,
    relative_tolerance: float = 1e-6,
) -> GlobalBoundTrace:
    """Bound the finite-frontier global optimum by convex-hull duality.

    For any field vector v, tangent convexity gives
    ||y||^2 >= 2 v^T y - ||v||^2.  Exact linear MCKP minimization of the
    right-hand side is therefore a valid lower bound over every discrete
    group allocation.  Frank-Wolfe line search improves v while preserving
    validity at every iteration.
    """
    weighted = _weighted_option_features(frontiers, option_features, router_weights)
    selected = np.asarray(upper_allocation.option_indices, np.int64)
    if selected.shape != (len(frontiers),):
        raise ValueError("upper allocation does not match frontiers")
    upper_residual = sum(
        (weighted[e][int(selected[e])] for e in range(len(frontiers))),
        np.zeros(weighted[0].shape[1], np.float64),
    )
    upper = float(upper_residual @ upper_residual)
    if not np.isclose(upper, float(upper_allocation.objective), rtol=1e-9, atol=1e-9):
        raise ValueError("upper allocation objective disagrees with its residual")
    iterate = upper_residual.copy()
    best_lower = 0.0
    completed = 0
    for iteration in range(int(max_iterations)):
        scores = tuple(2.0 * (values @ iterate) for values in weighted)
        linear = _linear_multiple_choice_allocate(
            frontiers, scores, int(page_budget),
        )
        lower = float(linear.objective - iterate @ iterate)
        best_lower = max(best_lower, min(upper, lower))
        completed = iteration + 1
        gap = max(upper - best_lower, 0.0)
        if gap <= float(relative_tolerance) * max(upper, 1e-30):
            break
        atom = sum(
            (
                weighted[e][int(linear.option_indices[e])]
                for e in range(len(frontiers))
            ),
            np.zeros_like(iterate),
        )
        direction = atom - iterate
        denominator = float(direction @ direction)
        if denominator <= 1e-30:
            break
        step = float(np.clip(-(iterate @ direction) / denominator, 0.0, 1.0))
        if step <= 1e-15:
            break
        iterate += step * direction
    absolute = max(upper - best_lower, 0.0)
    relative = absolute / max(upper, 1e-30)
    return GlobalBoundTrace(
        lower_bound=float(best_lower),
        upper_bound=upper,
        absolute_gap=absolute,
        relative_gap=relative,
        iterations=completed,
        certified=relative <= float(relative_tolerance),
    )
